  ...
  <div class="grid">
    <div class="card col-6 music">
      ...
      <div class="quote">
        ...
        <div class="quote-author">— Marc Aurele</div>
      </div>
    </div>
    <div class="card col-6 weather" id="weather">
      ...
      <div class="temps">
        <div class="temp">
          ...
          <div class="tval">14deg</div>
        </div>
        <div class="temp">
          <div class="tlab">&#x2600;&#xFE0F; Afternoon</div>
          <div class="tval">22deg</div>
        </div>
      </div>
...
export default function Home() {
  useEffect(() => {
    // Fetch real Pulse Score from Supabase
    fetch('/api/pulse')
      .then(r => r.json())
      .then(data => {
        const score = (data && data[0] && data[0].pulse_score) ? data[0].pulse_score : 0;
        initDashboard(score);
      })
      .catch(() => initDashboard(0));

    function initDashboard(pulseScore) {
      // Date
      const d = new Date();
      ...
      // Score chart
      (function() {
        const data = [58,61,60,64,62,59,63,66,64,68,65,63,67,70,69,66,64,68,71,73,70,68,72,74,71,69,73,72,70,pulseScore];
        const W=640, H=190, pad=14, max=100, min=0;
        const pts = data.map(function(v, i) {
          return [pad + i*(W-2*pad)/(data.length-1), H-pad-(v-min)/(max-min)*(H-2*pad)];
        });
