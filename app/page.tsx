...
const styleCss = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .hydra-bottle { background: none; border: none; padding: 0; cursor: pointer; transition: transform 0.2s; line-height: 0; }
  .hydra-bottle:active { transform: scale(0.92); }
  .hydra-bottle.done { transform: scale(1.12); }
  .hydra-bottle svg { display: block; }
  ...
  return '<div class="wrap">' +
    ...
    '<section class="hero">' +
      '<div class="hero-grid">' +
        ...
        '<div class="hero-row">' +
            '<div class="hydra-cta">' +
              '<button class="hydra-bottle" id="hydraBottle" title="Add 300 ml">' +
                '<svg viewBox="0 0 28 46" width="24" height="40">' +
                  ...
                  '<rect id="hydraFill" x="0" y="44" width="28" height="0" fill="#5bb8d4" clip-path="url(#bclip)"/>' +
                  '<path d="M10 6 L10 9 Q4 13 4 20 L4 40 Q4 44 8 44 L20 44 Q24 44 24 40 L24 20 Q24 13 18 9 L18 6 Z" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.8"/>' +
                '</svg>' +
              '</button>' +
    ...
    '<div class="grid">' +
      ...
      '</div>' +
'<div class="card col-4"><div class="card-head"><div class="card-title">Nutrition &mdash; yesterday</div><a href="/nutrition" class="card-go">Details &#x2192;</a></div><div class="macro"><div class="macro-top"><b>Calories</b><span class="muted" id="nCalTxt">-- / 2750 kcal</span></div><div class="track"><i id="nCalBar" style="width:0%; background:var(--gold)"></i></div></div><div class="macro"><div class="macro-top"><b>Protein</b><span class="muted" id="nProTxt">-- / 195 g</span></div><div class="track"><i id="nProBar" style="width:0%; background:var(--teal)"></i></div></div><div class="macro"><div class="macro-top"><b>Carbs</b><span class="muted" id="nCarTxt">-- / 285 g</span></div><div class="track"><i id="nCarBar" style="width:0%; background:#5bb8d4"></i></div></div><div class="macro"><div class="macro-top"><b>Fat</b><span class="muted" id="nFatTxt">-- / 92 g</span></div><div class="track"><i id="nFatBar" style="width:0%; background:#c47fd4"></i></div></div></div>' +
'</div>' +    
    '<div class="grid">' +
...
if (row.detail_texte && typeof row.detail_texte === 'string') {
  try { row.detail_texte = JSON.parse(row.detail_texte); } catch (e) {}
    ...
    function initDashboard(pulseScore, allData) {
      ...
      function paintHydration() {
        var total = window.__hydraTotal || 0;
        var goal = window.__hydraGoal || 3000;
        ...
        if (txt) txt.textContent = total + ' / ' + goal + ' ml';
      }
      ...
      function loadHydration() {
        fetch('/api/hydration')
          .then(function(r) { return r.json(); })
          .then(function(d) {
            window.__hydraTotal = d.total || 0;
            window.__hydraGoal = (d.goal || 3000) + ((window.__feels && window.__feels >= 35) ? 500 : 0);
            paintHydration();
          })
          .catch(function() {});
      }      
      ...
      if (hydraBottle) {
        hydraBottle.addEventListener('click', function() {
          if (hydraBottle.classList.contains('busy')) return;
          hydraBottle.classList.add('busy');

          window.__hydraTotal = (window.__hydraTotal || 0) + 300;
          paintHydration();
          hydraBottle.classList.add('done');
          setTimeout(function() { hydraBottle.classList.remove('done'); }, 900);

          fetch('/api/hydration', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ boisson: 'bouteille' })
          })
            .then(function(r) { return r.json(); })
            .then(function() {
              hydraBottle.classList.remove('busy');
              setTimeout(loadHydration, 1500);
            })
            .catch(function() {
              hydraBottle.classList.remove('busy');
              window.__hydraTotal -= 300;
              paintHydration();
            });
        });
      }
